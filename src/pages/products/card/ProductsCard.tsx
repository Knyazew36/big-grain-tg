...
const ProductsCard: FC<IProductsCard> = ({ title, updatedAt, min, count, ed }) => {
  return (
    <div
      className={clsx(
        ...
        count < min && '!border-red-500'
      )}
    >
      <div className='inline-flex justify-center items-center'>
        <span className={clsx('size-2 inline-block  rounded-full me-2', count < min ? 'bg-red-500' : 'bg-gray-500')} />
        <span className='text-xs font-semibold uppercase text-gray-600 dark:text-white'>{title}</span>
      </div>
      ...
      {/* <dl className='flex justify-center items-center divide-x divide-gray-200 dark:divide-neutral-800'>
        <dt className='pe-3'>
          <span className='text-green-600'>
            ...
            <span className='inline-block text-sm'>{min}</span>
          </span>
          <span className='block text-sm text-gray-500 dark:text-neutral-500'>минимально</span>
        </dt>
